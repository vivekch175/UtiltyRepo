...
from .serializers import  LoginSerializer,UserSerializer,PLCConnectionSerializer,TagSerializer,ScheduleGroupSerializer,TagHistorySerializer
from rest_framework import status,generics
from rest_framework.permissions import IsAuthenticated
from .models import CustomUser,PLCConnection,Tag,TagHistory,ScheduleGroup
        ...
        try:
            ...
            for attempt in range(retries + 1):
                if client.connect():
                    ...
                    print(f"Connected to {request.data['ip_address']}:{request.data['port']} on attempt {attempt + 1}")
                    break
                else:
                    print(f"Connection attempt {attempt + 1} failed for {request.data['ip_address']}:{request.data['port']}")
                if attempt < retries:
                    time.sleep(1)
            ...
            if connection_successful:
                ...
                print(f"Failed to connect to {request.data['ip_address']}:{request.data['port']} after {retries + 1} attempts")
        ...
        try:
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    print(f"Write attempt {attempt + 1} failed: {str(write_error)}")
            ...
            print(f"Unexpected error in ControlView: {e}")
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            PeriodicTask.objects.create(
                ...
                task='UtilityApp.tasks.execute_schedule',
